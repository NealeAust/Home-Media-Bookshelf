...
function DeleteMedia() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const { data } = await deleteMedia({
                variables: { mediaId: inputs.mediaId },
            });
            console.log(data);
    ...
    return (
        <div className="container-deleteMedia">
            ...
            <div>
                <h2>Media ID</h2>
                <input className="delMedia" name="mediaId" type="text" onChange={handleChange} />
                <button className="delMedia" id="submit" onClick={handleSubmit}>Submit</button>
            </div>
        </div>
    )
}
